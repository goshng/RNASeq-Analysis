...
\begin{abstract}
We analyze RNA samples from \textit{Streptococcus mutans}.  We use
...
approaches such as \program{BWA} \cite{Li2009a}.  Reads fitting into one of the annotated
...
which is a type provided by \Rpackage{DESeq}.  Using \program{BWA} \cite{Li2009a} and
...
\Rpackage{GenomicRanges} might be useful in 
<<tx,eval=FALSE>>=
...
Expression levels of \textif{fruA} in \textit{S.\ mutans} strains that were
grown to early exponential phase (OD_600, 0.2) in TV medium supplemented with
...
Now, I compared glucose and galactose for growing TW1. 
