...
x.big <- xtable( x[order(x[,"p-value"]),],
                 ...
                 caption='List of genes for the comparison UA159 and TW1 under glocuse-grown cells',
               )
print( x.big,
...
SMU.1424).  
\Sexpr{length(grep("Energy metabolism",x[,"Functional Class"]))}
\Sexpr{length(grep("PTS",x[,"Functional Class"]))}
\Sexpr{length(grep("hypothetical",x[,"Gene Description"]))}
...
%\caption[Illustration of recombinant tree]{
%{\bf Illustration of a recombinant tree in the ClonalOrigin model.}
%(A) A recombinant tree for our five-sample clonal frame with four
%recombinant edges (dashed lines with arrows running from donors to
%recipients).  (B) Genomic 
%regions associated with recombinant edges.  Recombination 
%intensities (number of edge types present) and gene tree labels are shown at
%bottom. (C)~Induced local gene trees.
%Bold lines indicate branches of the clonal frame, while dashed lines indicate
%recombinant edges. See section \textbf{Summary statistics of recombinant trees} in
%\textbf{Supplementary Text} for additional details.}
