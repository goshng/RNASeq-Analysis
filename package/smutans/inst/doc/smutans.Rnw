...
the detection of differentially expressed genes.  Tests of enrichment for
differentially expressed genes are performed using \Rpackage{goseq}.  The
package \Rpackage{smutans} contains the procedures.  This vignette explains the
use of the package. 
\end{abstract}
