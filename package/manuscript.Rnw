...
The resolution of RNA-Seq has advanced many aspects of the existing gene
annotation, including the determination of gene boundaries for known genes and
the identification of novel transcribed regions in many of the bacteria that
have previously been sequenced.
For example, for L. monocytogenes, 517 primary operons were identified that
encode 1719 genes, which represent 60% of the annotated genome
The knowledge gained regarding untranslated regulatory regions, operons and
sRNAs is triggering re-evaluation and re-annotation of many bacterial genomes.
