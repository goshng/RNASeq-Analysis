...
\fixme{Add more coauthors and change the order of authors if needed}, \\
Robert A.\ Burne$^{3}$
...
\fixme{Add corresponding authors if needed.}
...
information in one plot. We could label some of dots to emphasize. Done?}

% tableGlucoseOnly's functional classes can be multiple. We still need to have
% different colors using p-values.
% dotplot(tableGlucoseOnly[,"Functional Class"]~tableGlucoseOnly[,"fold"])

\fixme{Display results in UCSC genome browser. Done?}


\fixme{Young's GO analysis with the comparisons}
We used a q-value of $1\times 10^{-3}$ to determine differentially expressed
genes. Note that we had another q-value cutoff to list gene ontology terms for
functional category enrichment tests; 0.05 was the cutoff value for this.
We found gene ontology terms enriched with differentially expressed genes in the
comparison of UA159 and TW1 glucose-grown cells:
phosphotransferase system, 
glycogen biosynthetic process, 
sugar:hydrogen symporter activity, and
carbohydrate metabolic process (table \ref{tab:goseqGlucoseOnly}).
Because the number of differentially expressed genes in galactose-grown cells is
was smaller than that of glucose-grown cells, we found only one gene ontology
term, branched chain family amino acid biosynthetic process
(table \ref{tab:goseqGalactoseOnly}).
The comparisons between glucose-grown and galactose-grown UA159
(table \ref{tab:goseqUA159Only}), and that from TW1 mutant-type
(table \ref{tab:goseqTW1Only}) showed very similar functional categories. Ranks
of gene ontology terms were different. 
Lactose metabolic process was top in TW1, and
phosphoenolpyruvate-dependent sugar phosphotransferase system was top in UA159.

\fixme{Use KEGG for functional category study. Done?}
(table \ref{tab:kegseqGlucoseOnly})
(table \ref{tab:kegseqGalactoseOnly})
(table \ref{tab:kegseqUA159Only})
(table \ref{tab:kegseqTW1Only}) 

\fixme{Use two factor analysis. Done?}
Figure \ref{figClust}
Because the effects of nutrient changes appeared to be similar whether UA159
