...
\fixme{Refinements: More filtering steps in the short read alignments might be necessary. Done?}
...
% RNA-seq, and microarray, bacterial transcriptomes
...
mutant-type.  We attempted to define transcripts of mRNAs using the RNA-seq
data, and to find several small non-coding RNAs. Using the map of transcripts
...
\fixme{SCC wrote the paragraph based on email correspondences between PDPB and CDD.
Stanhope group describes RNA sequencing. Please add texts and SCC will copy-edit
them.}
...
# Copied from supplementary.Rnw. Compare the following with ones above to remove
# redundant lines.
load("smutans/data/smutansData.tx.RData")
load("smutans/data/smutansData.txGenes.RData")
load("smutans/data/smutansData.txGrCombined.RData")
...
This was silmilar to the comparison between UA159
