...
Table \ref{tabUA159Only}.  
Table \ref{tabTW1Only}.  

\fixme{Let's compare the microarray result and that from RNA-seq.}
\fixme{Use KEGG for functional category study.}
\fixme{Display results in UCSC genome browser.}
\fixme{Let's find strand-specific RNA-seq.}
\fixme{Add gene descriptions and functional category in the result of DE study.}
