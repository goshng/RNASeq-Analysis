...
removedCountsTable <- countsTable[countsTable[,4]+countsTable[,5]+countsTable[,6]+countsTable[,7]+countsTable[,8]+countsTable[,9]<=15,]
y <- length(removedCountsTable[,1])
cat ("Number of genes removed is ", y, "\n",sep="")
countsTable <- countsTable[countsTable[,4]+countsTable[,5]+countsTable[,6]+countsTable[,7]+countsTable[,8]+countsTable[,9]>15,]
...
postscript ("output/cornell/1/bwa/omz-de.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
plotDE(res)
y <- dev.off()

cat ("SCV file: omz-scv.ps\n")
postscript ("output/cornell/1/bwa/omz-scv.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
scvPlot(cds)
y <- dev.off()
