...
removedCountsTable <- countsTable[countsTable[,2]+countsTable[,3]+countsTable[,14]+countsTable[,15]+countsTable[,16]+countsTable[,17]<=15,]
y <- length(removedCountsTable[,1])
cat ("Number of genes removed is ", y, "\n",sep="")
countsTable <- countsTable[countsTable[,2]+countsTable[,3]+countsTable[,14]+countsTable[,15]+countsTable[,16]+countsTable[,17]>15,]
...
res <- nbinomTest(cds, "TW1GLU", "TW1GAL")
# 2 3 14 15 16 17
# 1  "UA159GLU"
# 2  "TW1GLU"
# 3  "TW1GAL"
...
cat ("SCV file: tw1-glu-vs-gal-de.ps\n")
postscript ("output/cornell/1/bwa/tw1-glu-vs-gal-de.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
plotDE(res)
y <- dev.off()

cat ("SCV file: tw1-glu-vs-gal-scv.ps\n")
postscript ("output/cornell/1/bwa/tw1-glu-vs-gal-scv.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
scvPlot(cds)
y <- dev.off()
