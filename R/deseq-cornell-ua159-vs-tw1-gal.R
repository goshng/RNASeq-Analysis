...
removedCountsTable <- countsTable[countsTable[,12]+countsTable[,13]+countsTable[,16]+countsTable[,17]+countsTable[,18]+countsTable[,19]<=15,]
y <- length(removedCountsTable[,1])
cat ("Number of genes removed is ", y, "\n",sep="")
countsTable <- countsTable[countsTable[,12]+countsTable[,13]+countsTable[,16]+countsTable[,17]+countsTable[,18]+countsTable[,19]>15,]
...
res <- nbinomTest(cds, "UA159GAL", "TW1GAL")
# 12 13 16 17 18 19
# 1  "UA159GLU"
# 2  "TW1GLU"
# 3  "TW1GAL"
...
cat ("SCV file: ua159-vs-tw1-gal-de.ps\n")
postscript ("output/cornell/1/bwa/ua159-vs-tw1-gal-de.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
plotDE(res)
y <- dev.off()

cat ("SCV file: ua159-vs-tw1-gal-scv.ps\n")
postscript ("output/cornell/1/bwa/ua159-vs-tw1-gal-scv.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
scvPlot(cds)
y <- dev.off()
