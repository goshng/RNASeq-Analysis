...
removedCountsTable <- countsTable[countsTable[,26]+countsTable[,27]+countsTable[,28]+countsTable[,29]+countsTable[,30]<=12.5,]
y <- length(removedCountsTable[,1])
cat ("Number of genes removed is ", y, "\n",sep="")
countsTable <- countsTable[countsTable[,26]+countsTable[,27]+countsTable[,28]+countsTable[,29]+countsTable[,30]>12.5,]
...
res <- nbinomTest(cds, "UA159noCSP", "UA159CSP")
# 26 27 vs 28 29 30
# 1  "UA159GLU"
# 2  "TW1GLU"
# 3  "TW1GAL"
...
cat ("SCV file: ua159-csp-de.ps\n")
postscript ("output/cornell/1/bwa/ua159-csp-de.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
plotDE(res)
y <- dev.off()

cat ("SCV file: ua159-csp-scv.ps\n")
postscript ("output/cornell/1/bwa/ua159-csp-scv.ps",  width=10, height=10, horizontal = FALSE, onefile = FALSE,
paper = "special")
scvPlot(cds)
y <- dev.off()
