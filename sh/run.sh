...
function convert-sra-to-fastq {
  fastq-dump -DB "@\$ac.\$si" -DQ "+\$ac.\$si" -A $1 -D $HyploriREADDIR/$1/$1.sra -O output
...
function prinseq-a-read {
  perl $PRINSEQ -fastq output/$1.fastq \
    -out_format 1 \
    -out_good output/$1 \
    -out_bad null \
    -min_len 12 \
...
CHOICES=( index-genome copy-data run-map srp001481 build-reads prinseq-reads filter-reads build-index match-reads )
